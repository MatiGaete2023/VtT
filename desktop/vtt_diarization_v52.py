...
class DiarizationEngine(v5.DiarizationEngine):
    ...
    def _light_identity(self, turnos, context, *, max_total=PRECHECK_MAX_EMBEDDINGS):
        ...
        analysis = v4.analizar_turnos(refined, float(context.get("audio_seconds", 0.0) or 0.0))
        return {
    ...
    def _auto_pre_retry(self, context: Dict[str, Any]):
        ...
        if reason not in {"sobredeteccion_extrema", "fragmentacion_excesiva"}:
            return {"accepted": False, "meta": {"enabled": False, "reason": "precheck_no_aplicable"}}
        result = self._light_identity(context["first_turns"], context)
        if result is None:
            return {"accepted": False, "meta": {"enabled": False, "reason": "audio_no_reutilizable"}}
    ...
    def _auto_choose_candidates(self, first, second, reason, context):
        struct_turns, struct_analysis, struct_reason = v4.elegir_candidato(first, second, reason)
        a = first[1]; b = second[1]
        structural_gap = abs(float(a.get("penalty", 0.0)) - float(b.get("penalty", 0.0)))
        ...
        score_a = float(first_id["analysis"].get("penalty", 0.0)) + float(first_id["identity_penalty"])
        score_b = float(second_id["analysis"].get("penalty", 0.0)) + float(second_id["identity_penalty"])
        ...
        return chosen["turns"], chosen["analysis"], f"seleccion_identity_aware_{which}", extra
    ...
    def diarize(
        ...
        context = {
            ...
            "audio_seconds": len(self._last_decoded_audio) / float(self._last_decoded_audio_rate)
            if self._last_decoded_audio is not None and self._last_decoded_audio_rate else 0.0,
        }
        rt = self._runtime(context)
        if rt is None:
            # Compatibilidad defensiva: la configuración actual de sherpa usa 16 kHz.
            import vtt_diarization as legacy
            t_dec = time.perf_counter(); audio = legacy.decodificar_audio_mono(ruta, 16000)
            decode_seconds = time.perf_counter() - t_dec
            context.update({"original_audio": audio, "sample_rate": 16000,
        ...
        if is_auto:
            ...
            refine_meta = {"skipped": True, "reason": "conteo_manual_preserva_identidades_sherpa",
                           "speakers_before": n0, "speakers_after": n0,
                           "reassigned_turns": 0, "new_identities": 0}
        local_enabled = bool(is_auto and any(
            float(t.get("end", 0.0)) - float(t.get("start", 0.0)) >= identity.LOCAL_SCAN_MIN_SECONDS
            for t in refined
        ))
        ...
        meta["identity_verification"] = {
            ...
            "final_stage_new_embedding_calls": int(rt["embedding_calls"]) - before_final_calls,
        ...
        meta["identity_wall_seconds"] = float(final_wall) + float((meta.get("auto_precheck") or {}).get("wall_seconds", 0.0) or 0.0)
        ...
        log(
            "V5.2 identidad: "
            f"{meta['detected_speakers_before_identity']} → {meta['detected_speakers']} cluster(s) · "
        ...
        self._last_decoded_audio = None; self._last_decoded_audio_path = None
        self._last_decoded_audio_rate = None; self._v52_runtime = None
        return scanned, meta
