    ...
    while True:
        ...
        try:
            turnos, meta = engine.diarize(
                str(cmd["ruta"]),
                num_speakers=int(cmd.get("num_speakers", -1)),
...
class PersistentDiarizationService:
    """Un proceso por aplicación; se reinicia solo al cancelar o fallar."""
    ...
    def _start(self) -> None:
        if self.proc is not None and self.proc.is_alive():
            return
        self.comandos = self.ctx.Queue()
        self.eventos = self.ctx.Queue()
        self.proc = self.ctx.Process(
            target=_worker_loop,
            args=(str(self.carpeta_modelos), self.comandos, self.eventos),
            daemon=True,
        )
        self.proc.start()
        ...
        while True:
            ...
            try:
                ...
                if self.proc is None or not self.proc.is_alive():
                    ...
                    self._reset()
                    raise RuntimeError(
                        f"El worker persistente de diarización terminó inesperadamente ({code})."
                    )
                continue
            if jid != job_id:
                continue
